...
function BlogCategory() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`/api/categoryupdate/${editingId}`, category);
        toast.success("Category updated successfully", {
          position: "bottom-right",
          autoClose: 1000,
        });
      } else {
        await axios.post("/api/blogcategorypost", category);
        toast.success("Category created successfully", {
          position: "bottom-right",
          autoClose: 1000,
        });
      }
      const categoryRes = await axios.get("/api/allcategorydata");
  ...
  let deletedata = async (id) => {
    try {
      await axios.delete(`/api/categoriesdelete/${id}`, cates);
    ...
    const fetchData = async () => {
      try {
        const [categoryRes, blogRes] = await Promise.all([
          axios.get("/api/allcategorydata"),
          axios.get("/api/blogpostdata"),
        ]);
        setCates(categoryRes.data || []);
