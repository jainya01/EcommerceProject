...
function ProductCategory() {
  ...
  const fetchCategories = async () => {
    try {
      const res = await axios.get("/api/productcatdata");
      setCate(res.data);
    } catch (err) {
    ...
    try {
      ...
      if (editingId) {
        response = await axios.put(
          `/api/categoriesupdate/${editingId}`,
          formData,
          {
            headers: {
        ...
        response = await axios.post("/api/product-category", formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
      ...
      if (response.status === 200 && response.data.success) {
        ...
        const categoryRes = await axios.get("/api/productcatdata");
    ...
    async function fetchData() {
      try {
        const [catRes, prodRes] = await Promise.all([
          axios.get("/api/productcatdata"),
          axios.get("/api/productpagedata"),
        ]);
        setCate(catRes.data || []);
  ...
  const deletedata = async (id) => {
    try {
      await axios.delete(`/api/categoriesdelete/${id}`);
