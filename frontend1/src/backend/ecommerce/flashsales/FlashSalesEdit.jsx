...
function FlashSalesEdit() {
  ...
  let handleSubmit = async () => {
    try {
      ...
      const response = await axios.put(`/api/updateflashsales/${id}`, payload);
  ...
  let somedata = async () => {
    try {
      const response = await axios.get(`/api/flashsalessome/${id}`);
    ...
    (async () => {
      try {
        ...
        const { data } = await axios.get(
          `/api/productpagedata?search=${encodeURIComponent(search)}`
        );
        setProducts(data);
      } catch {
